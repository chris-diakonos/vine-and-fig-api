...
class ElevationViewGenerator:
    ...
    def generate_elevation_svg(
        ...
        # Create foundation blocks using CadQuery
        foundation_blocks = CadQueryDrawingComponents.create_foundation_blocks_2d(
            x=offset_x,
            y=foundation_y,
        ...
        wp = wp.union(foundation_outline)
        
        # === WALLS WITH SHEATHING ===
        wall_y = offset_y + roof_rise
        
        # Main wall rectangle
        main_wall = CadQueryDrawingComponents.create_wall_2d(
            x=offset_x,
            y=wall_y,
            width=building_width,
        ...
        if sheathing:
            ...
            for i in range(1, num_boards):
                ...
                if sheathing_y < wall_y + building_height:
                    sheathing_line = cq.Workplane("XY").line((offset_x, sheathing_y), (offset_x + building_width, sheathing_y))
                    wp = wp.union(sheathing_line)
        
        # Corner boards (vertical trim at edges)
        corner_board_width = 4
        left_corner_board = CadQueryDrawingComponents.create_wall_2d(
            x=offset_x,
            y=wall_y,
            width=corner_board_width,
            height=building_height,
            wall_thickness=corner_board_width
        )
        right_corner_board = CadQueryDrawingComponents.create_wall_2d(
            x=offset_x + building_width - corner_board_width,
            y=wall_y,
            width=corner_board_width,
            height=building_height,
            wall_thickness=corner_board_width
        )
        wp = wp.union(left_corner_board).union(right_corner_board)
                ...
                if story_idx < len(windows):
                    ...
                    if isinstance(window_config, dict):
                        ...
                        for bay_x in bay_positions:
                            ...
                            # Use CadQuery window elevation drawing
                            window_2d = CadQueryDrawingComponents.create_window_elevation_2d(
                                x=window_x,
                                y=window_y,
                                width=window_width,
        ...
        if doors:
            for door in doors:
                if isinstance(door, dict) and door.get('wall') == face:
                    ...
                    # Use CadQuery door elevation drawing
                    door_2d = CadQueryDrawingComponents.create_door_elevation_2d(
                        x=door_x,
                        y=door_y,
                        width=door_width,
                        height=door_height,
