...
class CorniceSceneGraphTest(unittest.TestCase):
    def _load_request(self, path: Path) -> BuildingRequest:
        with open(path, "r", encoding="utf-8") as handle:
            return BuildingRequest(**json.load(handle))

    def _cornice_only_visibility(self) -> ComponentVisibility:
    ...
    def test_cornice_emits_scene_metadata_and_validation(self):
        ...
        self.assertTrue(any(path.startswith("building/cornice/front_cornice/") for path in paths))
        self.assertTrue(any(path.startswith("building/cornice/rear_cornice/") for path in paths))
